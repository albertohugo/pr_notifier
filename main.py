...
import webbrowser
import threading
import json import pystray 
from PIL import Image, ImageDraw 
from pystray import MenuItem as item 
import os
from winotify import Notification
import asyncio 
from winlitoast import toast
...
def get_open_prs():
  ...
  except ValueError as json _err:
    print(f'JSON decode error: {json_err}')
    print(f' Response content: {response.content}')
  return []

async def show _custom_toast(title, msg): 
  await toast(title, msg, duration=40)

async def show_toast_pr(pr):
  await toast(
    pr["title"] + " #" + str(pr["number"]),
    " by " + pr["user"] ["login"],
    button={
      'activationType': 'protocol'
      'arguments': pr["html_url"],
      'content': "Open Pull Request"
    }
def async_show_toast(toast):
  try:
  ...
  try:
    loop.run_until_complete(toast)
  except Exception as e:
    print(f"Error showing toast: {e)")

def notify_new_pr(pr):
  if notify:
    async_show_toast(show_toast_pr(pr))
...
def on_edit config(icon, item):
  def show modal():
    def save_config(new _config):
      with open('config-json', 'w') as config file:
        json.dump (new_config, config file, indent=4)
    ...
    api_url_entry.insert(0, configl 'api _url'])
    ...
    pr_url_entry.insert(0, configl'pr_ur1'])
    ...
    def on_save():
      new api_url = api_url_entry.get()
      new pr_url = pr_url_entry.get()
      new_ token = token_entry.get()
      new loop_duration = loop_duration_entry.get()
      notify_new_pr = notify_new_pr_var.get()
      
      if new api_url and new_token and new_loop_duration:
        config['api_url'] = new_api_url
        config['pr_url'] = new pr_url
        configl'token'] = new_token
    ...
    save_button.grid(row=5, column=0, columnspan=2, pady=10) top-mainloop)
    new_pr.destroy()
    threading.Thread(target=show_modal).start()

def main():
  global token
  icon _image = Image.open("resources/github.png")
